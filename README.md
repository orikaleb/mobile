...
1. Create a project in the [Firebase Console](https://console.firebase.google.com/), add an **Android** app whose **package name equals** `defaultConfig.applicationId` in `app/build.gradle.kts` (currently **`com.example.nexiride2`**), and download **`google-services.json`** into the **`app/`** module (replace the committed file). Under **Project settings → Your apps**, add your **debug SHA-1** certificate; otherwise Google may reject the API key at runtime (“API key not valid”). If you previously registered only **`com.nextride.app`**, add a **second** Android app for **`com.example.nexiride2`** in the same project and use the downloaded JSON so `mobilesdk_app_id` matches this package.
2. Enable **Authentication → Sign-in method → Email/Password**.
3. Create a **Cloud Firestore** database (start in test mode if you like, then deploy rules). Copy **`firestore.rules`** from this repo and run **`firebase deploy --only firestore:rules`** (or paste the rules in the **Firestore → Rules** editor and **Publish**). The committed rules are **dev-oriented** (open writes on **`routes`** / **`cities`** / **`seats`** so the in-app debug seed can run before sign-in); **tighten** these before any production release. If sign-up shows **`PERMISSION_DENIED`**, the console rules usually still **deny** `users/{uid}` writes—publish the repo rules (or equivalent) so authenticated users can **create** their own profile document.
